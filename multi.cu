...
#define numBodysperBlock 256 // p <= N/40 
#define numThreadsPerBody 4 //  numBodysperBlock * numThreadsPerBody,  pxq <= 1024
...
#define EPS2 0.000032 // softening parameter
...
__device__ float4 computeAcceleration(float4 bi, float4 bj, float4 ai) {
    float3 r;
...
__device__ float4 tileCalculation(float4 myPos, float4 accel) {
    ...
    for (int i = 0; i < blockDim.x; i++) {
        accel = computeAcceleration(myPos, shPosition[i+blockDim.x*threadIdx.y], accel);
    }
    return accel;
}
...
#define SX_SUM(i,j) shPosition[i+blockDim.x*j]
__global__ void forceCalculation(void *d_pos, void *d_acc, int numBodys) { 
    ...
    int p = blockDim.x; // number of threads per block
    int q = blockDim.y; // number of threads per body
    int n = numBodys; // number of bodies
    ...
    int gtid = WRAP(blockIdx.x+tile, gridDim.x) * blockDim.x + threadIdx.x; // global body id
    myPos = globPos[gtid];

    for (int i = start; i < end; i += p, tile++) {

        // int idx = tile * p + threadIdx.x;
        // if (threadIdx.x < p) {
        //     shPosition[threadIdx.x] = globPos[idx];
        // }
        shPosition[threadIdx.x+blockDim.x*threadIdx.y] = globPos[(WRAP(blockIdx.x+tile, gridDim.x)*blockDim.y + threadIdx.y )* blockDim.x + threadIdx.x];
        __syncthreads();
    ...
    if (threadIdx.y == 0) {
        ...
        ((float4*)d_acc)[blockIdx.x * p + threadIdx.x] = acc;
    } */ 
...
__global__ void updatePositions(int n, float4 *pos, float4 *vel, float4 *acc, double dt) { 
...
int main() { 
    ...
    FILE *fp = fopen("multi_output.csv", "w");
    if (!fp) {
    ...
    dim3 threads(numBodysperBlock, numThreadsPerBody, 1);
    dim3 grid(N / numBodysperBlock, 1, 1);
    int sharedMemSize = numBodysperBlock * numThreadsPerBody * sizeof(float4);

    clock_t kernel_in = clock();
    for (int step = 0; step < STEPS; step++) {
        // Calculate forces and update accelerations
        forceCalculation<<<grid, threads, sharedMemSize>>>(d_pos, d_acc_new, N);
        
        // Update positions
        updatePositions<<<grid, threads>>>(N, d_pos, d_vel, d_acc_old, DT);
        
        // Update velocities
        updateVelocities<<<grid, threads>>>(N, d_vel, d_acc_old, d_acc_new, DT);
